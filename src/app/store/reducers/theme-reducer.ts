import {TTheme} from "@Global/model";
import {Action, INIT} from "@ngrx/store";

export interface AppTheme {
    readonly theme: TTheme;
}

export const UPDATE_THEME = 'UPDATE_THEME';

export class UpdateTheme implements Action {
  type: string = UPDATE_THEME

  constructor(public updatedTheme: TTheme | undefined) {
  }
}

// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const THEMES : TTheme[] = [
  {name: 'ecommerce-kit-light-theme', displayName: 'Light', isDark: false},
  {name: 'ecommerce-kit-dark-theme', displayName: 'Dark', isDark: true},
];

export function themeReducer(state: TTheme = THEMES[0], action: Action): TTheme {
  switch(action.type) {
    case UPDATE_THEME:
      return <TTheme>(action as UpdateTheme).updatedTheme;
    case INIT:
      return THEMES[0]
    default:
      return state
  }
}
